...
if [ ! "$1" = "clean" ]; then
	ZIP="void-kernel-$(cat version)-$(git rev-parse --short HEAD)-n7100.zip"
	EXCLUDE=".gitignore modules/placeholder"

	if [ ! "$1" = "zip" ]; then
		if [ ! "$1" = ".config" ]; then make ARCH=arm n7100_defconfig; fi
		make -j16 ARCH=arm CROSS_COMPILE="../gcc-arm-none-eabi-4_9-2015q3/bin/arm-none-eabi-"; fi
