...
cp -rp ramdisks/samsung/jellybean/e210k/* ramdisks/samsung/jellybean/
...
echo "SHV-E210K" > ro.hardware

find . -name "*smdk4x12*" | grep -v asset | while read file; do mv -f "$file" "$(echo $file | sed s/smdk4x12/SHV-E210K/g)"; done
find . -name "*rc*" | grep -v asset | while read file; do sed -i s/smdk4x12/SHV-E210K/g $file ; done
...
tools/mkbootimg/mkbootimg --kernel $KERNELDIR/arch/arm/boot/zImage --ramdisk $RAMFS_TMP.cpio.lzma --board smdk4x12 --cmdline 'ttySAC2,115200 enforcing=0' --base 0x40000000 --pagesize 2048 -o $KERNELDIR/e210k.img
if [ "${1}" = "CC=\$(CROSS_COMPILE)gcc" ] ; then
	dd if=/dev/zero bs=$((8388608-$(stat -c %s e210k.img))) count=1 >> e210k.img
fi
