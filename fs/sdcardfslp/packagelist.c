...
int check_caller_access_to_name(struct inode *parent_node, const char* name,
	...
	if (parent_node && SDCARDFSLP_I(parent_node)->perm == PERM_ROOT) {
	...
	if (w_ok) {
		if (parent_node &&
			(current_fsuid() == SDCARDFSLP_I(parent_node)->d_uid)) {
			return 1;
		}
		return has_rw;
	}
...
static int insert_str_to_int(struct packagelist_data *pkgl_dat, void *key, int value) {
	...
	//printk(KERN_INFO "sdcardfslp: %s: %s: %d, %u\n", __func__, (char *)key, value, hash);
	hash_for_each_possible(pkgl_dat->package_to_appid, hash_cur, hlist, hash, h_n) {
...
static int insert_int_to_null(struct packagelist_data *pkgl_dat, void *key, int value) {
	...
	//printk(KERN_INFO "sdcardfslp: %s: %d: %d\n", __func__, (int)key, value);
...
static int read_package_list(struct packagelist_data *pkgl_dat) {
	...
	printk(KERN_INFO "sdcardfslp: read_package_list\n");
	...
	if (fd < 0) {
		printk(KERN_ERR "sdcardfslp: failed to open package list\n");
		mutex_unlock(&pkgl_dat->hashtable_lock);
		return fd;
	}
...
static int packagelist_reader(void *thread_data)
{
	...
	if (nfd < 0) {
		printk(KERN_ERR "sdcardfslp: inotify_init failed: %d\n", nfd);
		return nfd;
	}
	...
	while (!kthread_should_stop()) {
		...
		if (!active) {
			...
			if (res < 0) {
				...
				/* Framework may not have created yet, sleep and retry */
					printk(KERN_ERR "sdcardfslp: missing packages.list; retrying\n");
					ssleep(2);
					printk(KERN_ERR "sdcardfslp: missing packages.list_end; retrying\n");
					continue;
				} else {
					printk(KERN_ERR "sdcardfslp: inotify_add_watch failed: %d\n", res);
			...
			if (res) {
				printk(KERN_ERR "sdcardfslp: read_package_list failed: %d\n", res);
				goto interruptable_sleep;
			}
			active = true;
		}
		...
		if (res < (int) sizeof(*event)) {
			if (res == -EINTR)
				continue;
			printk(KERN_ERR "sdcardfslp: failed to read inotify event: %d\n", res);
			goto interruptable_sleep;
		}
		...
		while (res >= (int) sizeof(*event)) {
			...
			printk(KERN_INFO "sdcardfslp: inotify event: %08x\n", event->mask);
...
void * packagelist_create(gid_t write_gid)
{
	...
	if (!pkgl_dat) {
                printk(KERN_ERR "sdcardfslp: creating kthread failed\n");
	...
	printk(KERN_INFO "sdcardfslp: created packagelist pkgld/%d\n",
...
void packagelist_destroy(void *pkgl_id)
{
	...
	remove_all_hashentrys(pkgl_dat);
	printk(KERN_INFO "sdcardfslp: destroyed packagelist pkgld/%d\n", (int)pkgl_pid);
	kfree(pkgl_dat);
}
...
int packagelist_init(void)
{
	...
	if (!hashtable_entry_cachep) {
		printk(KERN_ERR "sdcardfslp: failed creating pkgl_hashtable entry slab cache\n");
		return -ENOMEM;
	}
