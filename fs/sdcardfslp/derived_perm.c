...
{
	struct sdcardfslp_inode_info *pi = SDCARDFSLP_I(parent);
	struct sdcardfslp_inode_info *ci = SDCARDFSLP_I(child);
...
{
	struct sdcardfslp_inode_info *info = SDCARDFSLP_I(inode);
...
inline void update_derived_permission(struct dentry *dentry)
{
	...
	if(!dentry || !dentry->d_inode) {
		printk(KERN_ERR "sdcardfslp: %s: invalid dentry\n", __func__);
		return;
	}
	/* FIXME:
...
{
	int ret = 0;
	struct sdcardfslp_dentry_info *di = SDCARDFSLP_D(dent);
	struct sdcardfslp_sb_info *sbi = SDCARDFSLP_SB(dent->d_sb);
	char *path_buf, *obbpath_s;

	/* check the base obbpath has been changed.
	 * this routine can check an uninitialized obb dentry as well.
	 * regarding the uninitialized obb, refer to the sdcardfslp_mkdir() */
	spin_lock(&di->lock);
...
int is_base_obbpath(struct dentry *dentry)
{
	...
	spin_lock(&SDCARDFSLP_D(dentry)->lock);
	/* DERIVED_LEGACY */
	...
	spin_unlock(&SDCARDFSLP_D(dentry)->lock);
	dput(parent);
	return ret;
}
...
int setup_obb_dentry(struct dentry *dentry, struct path *lower_path)
{
	...
	sdcardfslp_set_orig_path(dentry, lower_path);
