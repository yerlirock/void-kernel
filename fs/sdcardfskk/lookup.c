...
static struct kmem_cache *sdcardfskk_dentry_cachep;

int sdcardfskk_init_dentry_cache(void)
{
	sdcardfskk_dentry_cachep =
		kmem_cache_create("sdcardfskk_dentry",
				  sizeof(struct sdcardfskk_dentry_info),
				  0, SLAB_RECLAIM_ACCOUNT, NULL);

	return sdcardfskk_dentry_cachep ? 0 : -ENOMEM;
}

void sdcardfskk_destroy_dentry_cache(void)
{
	if (sdcardfskk_dentry_cachep)
		kmem_cache_destroy(sdcardfskk_dentry_cachep);
}
...
void free_dentry_private_data_kitkat(struct dentry *dentry)
{
	if (!dentry || !dentry->d_fsdata)
		return;
	kmem_cache_free(sdcardfskk_dentry_cachep, dentry->d_fsdata);
...
{
	struct sdcardfskk_dentry_info *info = SDCARDFSKK_D(dentry);

	/* use zalloc to init dentry_info.lower_path */
	info = kmem_cache_zalloc(sdcardfskk_dentry_cachep, GFP_ATOMIC);
	if (!info)
		return -ENOMEM;
...
static int sdcardfskk_inode_test(struct inode *inode, void *candidate_lower_inode)
{
	struct inode *current_lower_inode = sdcardfskk_lower_inode(inode);
...
static int sdcardfskk_inode_set(struct inode *inode, void *lower_inode)
{
	/* we do actual inode initialization in sdcardfskk_iget */
	return 0;
}
...
/*
 * Connect a sdcardfskk inode dentry/inode with several lower ones.  This is
 * the classic stackable file system "vnode interposition" action.
 *
 * @dentry: sdcardfskk's dentry which interposes on lower one
 * @sb: sdcardfskk's super_block
 * @lower_path: the lower path (caller does path_get/put)
 */
int sdcardfskk_interpose(struct dentry *dentry, struct super_block *sb,
		     struct path *lower_path)
{
	...
	/*
	 * We allocate our new inode below by calling sdcardfskk_iget,
	 * which will initialize some of the new inode's fields
	 */

	/* inherit lower inode number for sdcardfskk's inode */
	inode = sdcardfskk_iget(sb, lower_inode);
	if (IS_ERR(inode)) {
		err = PTR_ERR(inode);
		goto out;
	}
