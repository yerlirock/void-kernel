...
static inline void sdcardfskk_copy_inode_attr(struct inode *dest, const struct inode *src)
{
...
out_invalid:
	...
	printk(KERN_INFO "sdcardfskk reserved_mb : %u\n", sbi->options.reserved_mb);
	if (sbi->devpath)
		printk(KERN_INFO "sdcardfskk source path : %s\n", sbi->devpath);
