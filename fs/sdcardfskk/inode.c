...
#if 0
static int sdcardfskk_symlink(struct inode *dir, struct dentry *dentry,
			  const char *symname)
{
	int err = 0;
	struct dentry *lower_dentry;
...
static int touch(char *abs_path, mode_t mode) {
	...
	if (IS_ERR(filp)) {
		if (PTR_ERR(filp) == -EEXIST) {
			...
			printk(KERN_ERR "sdcardfskk: failed to open(%s): %ld\n",
			 ...
			 struct inode *new_dir, struct dentry *new_dentry)
{
	...
	sdcardfskk_copy_inode_attr(new_dir, lower_new_dir_dentry->d_inode);
	fsstack_copy_inode_size(new_dir, lower_new_dir_dentry->d_inode);
	fix_derived_permission(new_dir);
	if (new_dir != old_dir) {
		sdcardfskk_copy_inode_attr(old_dir, lower_old_dir_dentry->d_inode);
...
#if 0
static int sdcardfskk_readlink(struct dentry *dentry, char __user *buf, int bufsiz)
{
	int err;
	struct dentry *lower_dentry;
	struct path lower_path;
...
#if 0
static void *sdcardfskk_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	char *buf;
	...
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	err = sdcardfskk_readlink(dentry, buf, len);
	set_fs(old_fs);
	if (err < 0) {
		kfree(buf);
		buf = ERR_PTR(err);
...
static void sdcardfskk_put_link(struct dentry *dentry, struct nameidata *nd,
			    void *cookie)
{
...
const struct inode_operations sdcardfskk_symlink_iops = {
	.permission	= sdcardfskk_permission,
	.setattr	= sdcardfskk_setattr,
	/* XXX Following operations are implemented,
	 *     but FUSE(sdcard) or FAT does not support them
	 *     These methods are *NOT* perfectly tested.
	.readlink	= sdcardfskk_readlink,
	.follow_link	= sdcardfskk_follow_link,
	.put_link	= sdcardfskk_put_link,
	 */
};

const struct inode_operations sdcardfskk_dir_iops = {
	.create		= sdcardfskk_create,
	.lookup		= sdcardfskk_lookup,
	.permission	= sdcardfskk_permission,
	.unlink		= sdcardfskk_unlink,
	.mkdir		= sdcardfskk_mkdir,
	.rmdir		= sdcardfskk_rmdir,
	.rename		= sdcardfskk_rename,
	.setattr	= sdcardfskk_setattr,
	.getattr	= sdcardfskk_getattr,
	/* XXX Following operations are implemented,
	 *     but FUSE(sdcard) or FAT does not support them
	 *     These methods are *NOT* perfectly tested.
	.symlink	= sdcardfskk_symlink,
	.link		= sdcardfskk_link,
	.mknod		= sdcardfskk_mknod,
	 */
};

const struct inode_operations sdcardfskk_main_iops = {
	.permission	= sdcardfskk_permission,
	.setattr	= sdcardfskk_setattr,
	.getattr	= sdcardfskk_getattr,
};
